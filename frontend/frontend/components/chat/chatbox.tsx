...
// Hardwired fetcher directly to FastAPI
...
export function ChatBox() {
  ...
  const { data: history = [], mutate } = useSWR('/api/chat/history', fetcher)
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const errorMsg = errorData.detail || `Failed to send message (${response.status})`
  ...
  return (
    <div className="flex flex-col h-full bg-[#1a1a1a]">
      <div className="flex justify-end px-6 py-2 border-b border-gray-700">
        <Button onClick={handleClearHistory} variant="ghost" size="sm" className="text-gray-400 hover:text-white">
          Clear History
        </Button>
      </div>
      <div className="flex-1 overflow-y-auto px-6 py-8 space-y-6">
        {history.length === 0 ? (
          ...
          /* THE FIX: Added [...history].reverse() to flip the array so oldest messages are at the top */
                  ...
                  <div className="bg-[#2a2a2a] border border-gray-700 text-gray-100 rounded-2xl px-6 py-3">
                    ...
                    <div className="text-base leading-relaxed whitespace-pre-wrap">
                      <ReactMarkdown
                        components={{
                          a: ({node, ...props}) => (
                            <a {...props} target="_blank" rel="noopener noreferrer" className="text-teal-400 hover:text-teal-300 underline font-medium" />
                          ),
